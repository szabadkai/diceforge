...
function orientGeometry(geometry: THREE.BufferGeometry, frame: ReturnType<typeof getFaceFrames>[number]) {
  geometry.applyMatrix4(faceTransform(frame));
  return geometry;
}

function makePipCutters(config: DiceConfig, frames: ReturnType<typeof getFaceFrames>) {
  const cutters: THREE.BufferGeometry[] = [];
  frames.slice(0, config.sides).forEach((frame, faceIndex) => {
    const layout = pipLayout(config.values[faceIndex]);
    if (!layout.length) return;
    const radius = Math.min(frame.radius * 0.15, 0.92);
    layout.forEach(([x, y]) => {
      ...
      const position = frame.center
        .clone()
        .addScaledVector(frame.tangent, x * frame.radius * 2)
        .addScaledVector(frame.bitangent, y * frame.radius * 2)
...
function makeTextCutters(config: DiceConfig, frames: ReturnType<typeof getFaceFrames>) {
  const cutters: THREE.BufferGeometry[] = [];
  frames.slice(0, config.sides).forEach((frame, faceIndex) => {
    ...
    const geometry = new TextGeometry(value.slice(0, 3), {
      font,
      size: frame.radius * (value.length > 1 ? 0.72 : 0.95),
...
function makeGraphicCutters(config: DiceConfig, frames: ReturnType<typeof getFaceFrames>) {
  ...
  try {
    ...
    return frames.slice(0, config.sides).map((frame) => {
      ...
      const scale = (frame.radius * 1.25) / maxDimension;
      geometry.translate(
...
export async function buildDiceStl(config: DiceConfig): Promise<Blob> {
  ...
  const frames = getFaceFrames(baseGeometry);
